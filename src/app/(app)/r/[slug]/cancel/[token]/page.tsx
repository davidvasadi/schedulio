      ...
      <div className="text-center max-w-md">
        {state === 'done' ? (
          <>
            ...
            <h1 className="text-2xl font-black tracking-tight text-zinc-900 dark:text-white mb-2">Foglalás lemondva</h1>
            <p className="text-zinc-500 dark:text-white/50 mb-8">A foglalásodat sikeresen lemondtuk.</p>
          </>
        ) : (
          <>
            ...
            <h1 className="text-2xl font-black tracking-tight text-zinc-900 dark:text-white mb-2">Foglalás lemondása</h1>
            <p className="text-zinc-500 dark:text-white/50 mb-8">
              {state === 'error' ? 'A lemondás nem sikerült — lehet, hogy a link lejárt vagy érvénytelen.' : 'Biztosan le szeretnéd mondani a foglalásodat?'}
            </p>
            {state !== 'error' && (
              <button
                onClick={cancel}
              ...
              >
                {state === 'loading' ? <><Loader2 className="h-4 w-4 animate-spin" />Lemondás…</> : 'Igen, lemondom'}
              </button>
            )}
          </>
        )}
