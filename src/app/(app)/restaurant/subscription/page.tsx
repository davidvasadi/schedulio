...
const FEATURES = [
  'Korlátlan asztalfoglalás',
  'Termek és asztalok kezelése',
  'Részletes statisztikák és kihasználtság',
  'Email értesítések (foglalás, lemondás)',
  'Nyilvános foglalási oldal egyedi linken',
  'Asztaltérkép és turnus-kezelés',
  'Light / Dark mód',
...
export default async function RestaurantSubscriptionPage() {
  ...
  const isActivePro = sub?.status === 'active' && sub?.plan === 'restaurant_pro'
  const cancelScheduled = sub?.cancel_at_period_end === true
  const planLabel = sub?.plan === 'restaurant_pro' ? 'Étterem Pro' : 'Próbaidőszak'
  const priceLabel = sub?.plan === 'restaurant_pro' ? '9 900 Ft' : 'Ingyenes'
  ...
  return (
    <div className="p-5 lg:p-8 space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 lg:gap-3">
        ...
        <Kpi sub="Indulás" value={formatDate(restaurant?.createdAt as unknown as string)} label="regisztráció" />
      </div>
      ...
      <div className="bg-white shadow-sm border border-zinc-100 dark:bg-white/[0.04] dark:border-white/[0.08] dark:shadow-none rounded-2xl px-5 py-4 flex items-center gap-3">
        ...
        <p className="text-sm text-zinc-500 dark:text-white/50 flex-1">
          {sub?.status === 'trialing' && <>Próbaidőszakban vagy. <span className="text-zinc-900 dark:text-white font-bold">{days ?? 0} nap</span> múlva lejár — utána Étterem Pro csomagra kell váltani.</>}
          {isActivePro && !cancelScheduled && <>Aktív <span className="text-zinc-900 dark:text-white font-bold">Étterem Pro</span> előfizetésed van. Automatikus megújulás <span className="text-zinc-900 dark:text-white font-bold">{formatDate(periodEnd)}</span>-én.</>}
