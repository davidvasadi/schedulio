...
export function OpeningHoursEditor({
  ...
  return (
    <div className="space-y-5">
      {/* Hero — sötét kártya: cím + összegző, desktopon a 3 stat is */}
      <div className="relative overflow-hidden rounded-[26px] bg-ink-dark text-white p-5 lg:p-7 shadow-dav-card">
        ...
        <div className="relative">
          ...
          <h2 className="mt-1 text-2xl lg:text-3xl font-light tracking-[-0.02em]">Heti nyitvatartás</h2>
            ...
            >
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-ink">{DAY_LABELS_HU[d.day_of_week]}</p>
                  <p className="text-xs text-ink-soft">{d.is_open ? `${d.open_time} – ${d.close_time}` : 'Zárva'}</p>
                </div>
                <button
                  type="button"
                  onClick={(e) => { e.stopPropagation(); quickToggle(idx) }}
                  aria-label={d.is_open ? 'Zárás' : 'Nyitás'}
                  className={cn('relative h-6 w-10 shrink-0 rounded-full transition-colors', d.is_open ? 'bg-ink-dark' : 'bg-line-strong')}
                >
              ...
              <div className="hidden items-center gap-x-6 px-8 py-3.5 lg:grid lg:grid-cols-3">
                ...
                <div className="flex items-center gap-3">
                  <DayBadge day={d.day_of_week} />
                  <p className={cn('text-[15px] font-semibold', d.is_open ? 'text-ink' : 'text-ink-soft2')}>
                ...
                >
                  <span className={cn('relative h-6 w-10 shrink-0 rounded-full transition-colors', d.is_open ? 'bg-ink-dark' : 'bg-line-strong')}>
                    <span className={cn('absolute top-1 h-4 w-4 rounded-full bg-white shadow-sm transition-all', d.is_open ? 'left-5' : 'left-1')} />
                  </span>
                  <span className={cn('text-xs font-medium', d.is_open ? 'text-ink-soft' : 'text-ink-soft2')}>
                ...
                <div className="flex items-center justify-end">
                  {d.is_open ? (
                    <div className="flex items-center gap-3">
                      <TimeSelect value={d.open_time} onChange={(v) => update(idx, { open_time: v })} className="w-[7rem]" />
                      <span className="text-sm text-ink-soft2">–</span>
                      <TimeSelect value={d.close_time} onChange={(v) => update(idx, { close_time: v })} className="w-[7rem]" />
                    </div>
                  ) : (
      ...
      {dirtyCount > 0 && (
        <div className="hidden items-center justify-between gap-4 rounded-[22px] border border-amber-300/50 bg-amber-50 px-6 py-3.5 lg:flex">
      ...
      <Sheet open={editIdx != null} onOpenChange={(o) => { if (!o) setEditIdx(null) }}>
        <SheetContent side="bottom" className="rounded-t-[26px] border-t border-line bg-white max-h-[85vh] overflow-y-auto">
          {editIdx != null && (
            <div>
              <div className="flex items-center justify-between gap-3 mb-6">
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    <p className="font-semibold text-ink truncate">{DAY_LABELS_HU[days[editIdx].day_of_week]}</p>
              ...
              {days[editIdx].is_open ? (
                <>
                  <p className="text-[10px] font-semibold uppercase tracking-widest text-ink-soft2 mb-2">Idősáv</p>
                  <div className="flex items-center gap-3 mb-6">
                    <button type="button" onClick={() => setPicker('open')} className="flex-1 h-12 rounded-2xl bg-paper text-lg font-semibold tabular-nums text-ink">{days[editIdx].open_time}</button>
                    <span className="text-sm text-ink-soft2">–</span>
                    <button type="button" onClick={() => setPicker('close')} className="flex-1 h-12 rounded-2xl bg-paper text-lg font-semibold tabular-nums text-ink">{days[editIdx].close_time}</button>
                  </div>
                </>
              ) : (
                <p className="text-sm text-ink-soft2 mb-6">Ezen a napon zárva — nem fogadtok foglalást.</p>
              )}

              <p className="text-[10px] font-semibold uppercase tracking-widest text-ink-soft2 mb-2">Alkalmazás több napra</p>
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        ...
                        on ? 'bg-ink-dark text-white' : 'bg-paper text-ink-soft'
                      )}
                    >
                      {DAY_ABBR[day]}
                    </button>
              ...
              <div className="flex gap-3">
                <button type="button" onClick={() => setEditIdx(null)} className="flex-1 h-11 rounded-2xl border border-line-strong text-sm font-semibold text-ink-soft">Mégse</button>
                <button type="button" onClick={saveSheet} disabled={savingAll} className="flex-1 h-11 rounded-2xl bg-ink-dark text-sm font-semibold text-white disabled:opacity-60">{savingAll ? 'Mentés...' : 'Mentés'}</button>
              </div>
            </div>
          )}
        </SheetContent>
