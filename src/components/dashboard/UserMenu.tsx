...
import { Loader2, Plus, MoreHorizontal, Monitor, Sun, Moon, LogOut, CreditCard, Settings } from 'lucide-react'
import { cn } from '@/lib/utils'
import { UserAvatar } from './UserAvatar'

/**
 * A desktop sidebar aljára szánt fiók-blokk: avatar + név, mellette (children-ként)
 * a harang, és egy „⋯" gomb. Az avatarra VAGY a „⋯"-ra kattintva ugyanaz a felfelé
 * nyíló fiók-popover jelenik meg: profil + profilkép-szerkesztés, téma-váltó és
 * kijelentkezés. Összecsukott (collapsed) navban csak az avatar + „⋯" látszik.
 */
export function UserMenu({
  ...
  settingsHref?: string
  /** A harang (NotificationBell) — az avatar és a „⋯" gomb közé renderelődik. */
  children?: ReactNode
}) {
  ...
  return (
    <div ref={anchorRef} className={cn('flex items-center gap-1.5', collapsed && 'flex-col')}>
      ...
      >
        <UserAvatar name={name} src={avatarUrl} size={collapsed ? 32 : 36} />
        {!collapsed && (
          <span className="min-w-0 flex-1 text-left">
            <span className="block truncate text-sm font-semibold text-zinc-900 dark:text-white">{name ?? 'Fiók'}</span>
            {email && <span className="block truncate text-xs text-zinc-400 dark:text-white/30">{email}</span>}
          </span>
        )}
      </button>

      {/* A harang (children) az avatar és a „⋯" között. */}
      {children}

      {/* „⋯" gomb — szintén a fiók-popovert nyitja. */}
      <button
        type="button"
        onClick={toggleMenu}
        aria-label="Fiók beállítások"
        aria-expanded={open}
        className="shrink-0 flex items-center justify-center h-9 w-9 rounded-lg text-zinc-500 hover:text-zinc-900 hover:bg-zinc-100 dark:text-white/40 dark:hover:text-white dark:hover:bg-white/[0.06] transition-colors"
      >
      ...
      {open && pos && typeof document !== 'undefined' && createPortal(
        <>
          ...
          <div
            className="fixed w-64 max-w-[calc(100vw-1rem)] rounded-xl border border-zinc-100 bg-white shadow-lg dark:bg-zinc-950 dark:border-white/[0.08] z-[100] overflow-hidden"
          ...
          >
            ...
            <span className="min-w-0 flex-1">
              <span className="block truncate text-sm font-semibold text-zinc-900 dark:text-white">{name ?? 'Fiók'}</span>
              <span className="block truncate text-xs text-zinc-400 dark:text-white/30">{uploading ? 'Feltöltés…' : (email ?? '')}</span>
            </span>
          </button>
