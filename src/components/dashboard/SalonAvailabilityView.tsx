...
import { SalonAvailabilityExceptions, type SalonException } from '@/components/dashboard/SalonAvailabilityExceptions'
...
export function SalonAvailabilityView({
  salonId,
  days: initialDays,
  exceptions,
  bufferMinutes,
  ...
  // A rendes heti nyitvatartás (nap → nyitva/idők) — az Eltérő napok ezzel dönti el a feliratot.
  const weeklyHours = useMemo(
    () => Object.fromEntries(days.map((d) => [d.day_of_week, { is_open: d.is_available, open_time: d.start_time, close_time: d.end_time }])),
    [days],
  )
  ...
  // Mentés-kontraktus: POST/PATCH /api/availability (recurring:true, staff nélkül = szalon-szint).
  const persist = async (d: DayRow): Promise<DayRow> => {
    ...
    const res = d.id
      ? await fetch(`/api/availability/${d.id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, credentials: 'include', body: JSON.stringify(body) })
    ...
    <div className="p-5 lg:p-0">
      ...
      <div className="mt-6 grid grid-cols-1 gap-[5px] lg:grid-cols-[1.7fr_1fr] lg:items-start">
        ...
        <div className="flex flex-col gap-[5px]">
          ...
          <div className="rounded-[26px] dav-card-glass px-4 pb-4 pt-1 sm:px-6 sm:pb-5">
            ...
            {days.map((d, idx) => {
              ...
              const geo = d.is_available ? blockGeo(d.start_time, d.end_time) : null
              ...
              return (
                ...
                >
                  <div className="flex w-[104px] shrink-0 items-center gap-1.5">
                    <span className={cn('whitespace-nowrap text-sm', isToday ? 'font-semibold' : 'font-medium', d.is_available ? 'text-ink' : 'text-ink-soft2')}>
                  ...
                  <div className="relative h-7 flex-1 overflow-hidden rounded-[9px] bg-[#F1F0EB]">
                    ...
                    {d.is_available && geo ? (
                      <div
                        ...
                        style={{ left: `${geo.left.toFixed(2)}%`, width: `${geo.width.toFixed(2)}%`, background: '#26231F', boxShadow: '0 2px 6px rgba(40,35,15,.14)' }}
                      >
                        {showLabel ? `${d.start_time} – ${d.end_time}` : ''}
                      </div>
                    ) : (
                  ...
                  <Pencil className={cn('h-[15px] w-[15px] shrink-0', d.is_available ? 'text-ink' : 'text-[#C2BDAD]')} strokeWidth={1.6} />
                </button>
              )
            })}
          </div>
          ...
          <div className="rounded-[26px] dav-card-glass px-5 py-5 sm:px-6">
            ...
            <RuleRow label="Foglalási időköz" value={bufferMinutes != null ? `${bufferMinutes} perc` : '—'} />
        ...
        <div className="flex flex-col gap-[5px]">
          ...
          <SalonAvailabilityExceptions salonId={salonId} initial={exceptions} weeklyHours={weeklyHours} />
        </div>
      </div>

      {/* ── Nap-szerkesztő sheet ── */}
          ...
          {editIdx != null && (
            <div>
              <div className="mb-6 flex items-center justify-between gap-3">
                ...
                <button
                  type="button"
                  onClick={() => update(editIdx, { is_available: !days[editIdx].is_available })}
                  className={cn('relative h-6 w-10 shrink-0 rounded-full transition-colors', days[editIdx].is_available ? 'bg-ink-dark' : 'bg-line-strong')}
                >
                  <span className={cn('absolute top-1 h-4 w-4 rounded-full bg-white transition-all', days[editIdx].is_available ? 'left-5' : 'left-1')} />
                </button>
              </div>

              {days[editIdx].is_available ? (
                <>
                  <p className="mb-2 text-[10px] font-semibold uppercase tracking-widest text-ink-soft2">Idősáv</p>
                  <div className="mb-6 hidden items-center gap-3 lg:flex">
                    <TimeSelect value={days[editIdx].start_time} onChange={(v) => update(editIdx, { start_time: v })} className="w-[7rem]" />
                    <span className="text-sm text-ink-soft2">–</span>
                    <TimeSelect value={days[editIdx].end_time} onChange={(v) => update(editIdx, { end_time: v })} className="w-[7rem]" />
                  </div>
                  <div className="mb-6 flex items-center gap-3 lg:hidden">
                    <button type="button" onClick={() => setPicker('open')} className="h-12 flex-1 rounded-2xl bg-paper text-lg font-semibold tabular-nums text-ink">{days[editIdx].start_time}</button>
                    <span className="text-sm text-ink-soft2">–</span>
                    <button type="button" onClick={() => setPicker('close')} className="h-12 flex-1 rounded-2xl bg-paper text-lg font-semibold tabular-nums text-ink">{days[editIdx].end_time}</button>
                  </div>
                </>
              ) : (
      ...
      {editIdx != null && (
        <WheelTimePicker
          ...
          value={picker === 'close' ? days[editIdx].end_time : days[editIdx].start_time}
          onChange={(v) => editIdx != null && update(editIdx, picker === 'close' ? { end_time: v } : { start_time: v })}
          shorthands={picker === 'close' ? ['17:00', '18:00', '19:00', '20:00'] : ['08:00', '09:00', '10:00', '11:00']}
        />
      )}
    </div>
  )
...
/** Sötét NAP×ÓRA pötty-hőtérkép — az étterem „Tipikus terhelés" kártyájával azonos kinézet. */
